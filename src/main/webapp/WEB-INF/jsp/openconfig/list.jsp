            ...
            <table class="table table-striped">
              ...
              <c:forEach items="${entrys}" var="entry">
                  ...
                  <td>${entry.id}</td>
                  	...
                  	<a href="<%=request.getContextPath()%>/manage/openConfig/${entry.id}/load.do">编辑</a></td>
                </tr>
              </c:forEach>
              </tbody>
            </table>
...
function action(id,active) {
	...
	$.ajax({
		  type: 'GET',
		  url: "<%=request.getContextPath()%>/manage/openConfig/"+id+"/"+(active==1?"inactive":"active")+".do",
...
$(document).ready(function(){
	$(".add").click(function(){
		 window.location.href="<%=request.getContextPath()%>/manage/openConfig/0/load.do";
	});
});
</script>
</body>